...
void drawBoard(ChessWindow& window, const Chess& game, int squareSize) {
    ...
    for (int row = 0; row < 8; ++row) {
        for (int col = 0; col < 8; ++col) {
            ...
            if (piece != game.getEmpty()) {
                ...
                window.drawPiece(col * squareSize + offset, (7 - row) * squareSize + offset, squareSize, pieceChar,
                                 isBlack);
            }
        }
    }
    ...
    for (int col = 0; col < 8; ++col) {
        ...
        window.drawText(col * squareSize + squareSize / 2 + offset - 8, 8 * squareSize + offset + 24,
